...
class ProductViewModel {
    ...
    func orderMessageInfo(json: [String: String]) async throws -> BaseModel {
        ...
        do {
            let model: BaseModel = try await HttpRequestManager.shared.uploadWithForm("/considerablyreal/discovery", parameters: json)
            return model
        } catch {
            print("error===: \(error)")
            throw error
